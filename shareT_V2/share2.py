import time,threading,os,sqlite3,random,sys
global hosta,hostb,version
...
print(" \t\tBIENVENUE DANS SHARET\n\t by toutpuissantged \n\t\t contact :: mail :toutpuissant69@gmail.com  ; whatsapps+telegrame : +228 96870360\n \t\t\tversion : {} \n".format(version))
date=int(time.time())
expir=1689807440
rest=int(expir-date)
...
comp,inter=1,1
date="hi"
donnee=(comp , inter, date)
try:
  conn=sqlite3.connect("virtual/system/base.db")
  cur=conn.cursor()
  cur.execute("create table info2 (num integer ,scor integer ,dat )")
  cur.execute("insert into info2 (num,scor,dat) values( ?,?,?)",donnee)
  conn.commit()
  cur.close()
  conn.close()
  conn=sqlite3.connect("virtual/system/base.db")
            ...
            if msg2=="send":
                ...
                while neu:
                    ...
                    if tim3>=seuc:
                        ...
                        fd.close()
                        neu=0
                        print("reception de {} terminer avec un temps de {} secondes".format(msg3,tim4))
            elif msg2=="cmd":
                msg2=conn.recv(1024).decode("utf8")
                os.system(msg2)
            elif msg2=="exit":
...
def client():
   ...
   while inte ==0:
        ...
        if msg1=="send":
            while dev:
                try:
                    name=input(str(("entrer le nom du fichier : ")))
                    time.sleep(1)
